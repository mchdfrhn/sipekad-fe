...
const SuccessModal = ({ onOkHandler, text = "Pengajuan berhasil ditambahkan", isSuccess }) => {
  return (
    <>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.1, ease: ["easeInOut"] }} className="bg-white rounded-md w-xs relative flex justify-center flex-col gap-8 px-8 w-xs md:w-md xl:w-xl py-10">
            <div className="flex justify-center">
                <div className="size-20">
                  {
                    !isSuccess ? <CheckCircle className="w-full text-green-500 h-full" /> : <XCircle className="w-full text-red-500 h-full " />
                  }
                    
                </div>
            </div>
            <div className="text-center">
                <p className="text-2xl md:text-4xl font-semibold text-gray-800">{ !isSuccess ? text : "Anda telah mengajuakn pengajuan yang sama" }</p>
            </div>
            <div className="flex justify-center">
